...
{
    long_t val = 0;
    if(op == 0) val = argB + argA;
    if(op == 1) val = argB - argA;
    if(op == 2) val = argB & argA;
    if(op == 3) val = argB ^ argA;
    return val;
}
...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t ovf = 0;
    if(op == 0){
        ovf = ((argB<0)&&(argA<0)&&(val>0))||((argB>0)&&(argA>0)&&(val<0));
    }
    else if(op == 1){
         ovf = ((argB>0)&&(argA<0)&&(val<0))||((argB<0)&&(argA>0)&&(val>0));
    }   
...
{
    bool_t doit = TRUE;
    bool_t notdoit = FALSE;
    bool_t ZF = GET_ZF(cc);
    bool_t SF = GET_SF(cc);
    bool_t OF = GET_OF(cc);
    if(cond == C_YES)return doit;
    if((cond == C_LE)&&((ZF^OF)|ZF))return doit;
    if((cond == C_L)&&(SF^OF))return doit;
    if((cond == C_E)&&(ZF))return doit;
    if((cond == C_NE)&&(!ZF))return doit;
    if((cond == C_GE)&&(!(SF^OF)))return doit;
    if((cond == C_G)&&((!(SF^OF))&(!ZF)))return doit;
    return notdoit;
}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if(((icode >= 2)&&(icode <= 6))||(icode == I_POPQ)||(icode==I_PUSHQ)){     
        if (!get_byte_val(sim->m, next_pc, &codefun)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
    ...
    long_t valA,valB,addr;
    long_t valE,valM,valU; 
    bool_t Cnd;
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        ...
        addr = valB + IMME; 
        if(!set_long_val(sim->m,addr,valA)){
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, addr);
            //err_print("%rsp: 0x%016lx	0x%016lx", valB, valE);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        ...
        valB = get_reg_val(sim->r,regB);
        addr = valB + IMME;
        //get_long_val(sim->m,addr,&valE);
        if(!get_long_val(sim->m,addr,&valE)){
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, addr);
            //err_print("%rsp: 0x%016lx	0x%016lx", valB, valE);
            return STAT_ADR;
        }
      ...
      case I_JMP: /* 7:x imm */
        Cnd = cond_doit(sim->cc,ifun);
       // if ((IMME >= 0 )&& (IMME <= sim->m->len)){
            if(Cnd&&((IMME >= 0 )&& (IMME <= sim->m->len))){
                sim->pc = IMME;
            } 
            else{
                sim->pc = next_pc;
        ...
        /*
            ...
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, IMME);
        }
        */
        break;
      ...
      case I_RET: /* 9:0 */
        ...
        /*
        if(!get_long_val(sim->m,valE,&valU)){
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, valE);
                return STAT_ADR;
        }
        */
